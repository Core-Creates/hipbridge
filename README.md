...
**Status: pre-alpha.** Seven kernels are covered, spanning the row-wise part of
a transformer's inference path, and all seven verify on an MI300X. Per-kernel
case counts live in [results/verify-hipcc-gfx942.md](results/verify-hipcc-gfx942.md),
which is regenerated by CI rather than copied into prose here. Correctness has
been checked on both vendors: nvcc on an RTX 4060 via WSL2, and hipcc on an
MI300X (gfx942).
...
Seven kernels, which is the row-wise part of a transformer's inference path.
Each ships a naive original, a competently written HIP baseline to benchmark
...
| `rms_norm_rope` | `gamma`, `cos_tab`, `sin_tab` | `kernels.fused.rms_norm_rope_rowwise` |

All seven verify on an MI300X. Two things about the list are worth saying
...
The other fourteen are the seven covered kernels, each in a naive and a tuned
...
| AMD, hipcc on MI300X (gfx942) | **verified**, all seven suites (counts in `results/`) |
...
All seven kernels are verified and timed in float16 and bfloat16 as well as
